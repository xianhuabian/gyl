...
<script type="text/javascript">
	$().ready(function(){
	    //设置分页要跳转到的url
		$("body").data("url","supplierAction_showPageResult.action");
		//声明分页的事件
	    ...
	    GylUtils.basedata.deleteObj.deleteFunction({
			...
			url:'supplierAction_deleteSuppliers.action'
		});
		//$.deleteObjForm();
		//声明修改的事件
		GylUtils.basedata.updateObj.updateFunction({
			url:'supplierAction_updateUI.action',
			id:'sid'
		}); 
		//声明只删除一个的事件
		GylUtils.basedata.deleteObj.deleteOneFunction({
			url:'supplierAction_deleteOneSupplier.action',
			id:'sid'
		}); 
	});
</script>
...
<body>
<s:form action="supplierAction_deleteSuppliers.action">
<table id="userwrap">	
	<tr class="user">
        <td class="right">
        	<table cellspacing="0" cellpadding="0" id="maintain-top" align="center"  width="900" style=" overflow:auto; height:40px;">
                ...
                <tr class="one" height="20">
                    ...
                    <td width="300">供应商名称</td>
                    <td width="300">地址</td>
					<td width="300">电话</td>
					<td width="300">操作</td>
                </tr>
               	<s:iterator value="#suppliers.rows">
	                <tr align="center">
	...
	<span class="page" style="float:right">
		<input type="button" param="1" flag="firstPage" value="首页"/>&nbsp;<input type="button" flag="prePage" param="${suppliers.currentPage-1}" value="上一页" />&nbsp;<input type="button" param="${suppliers.currentPage+1}" flag="nextPage" last="${suppliers.totalPages}" value="下一页"/>&nbsp;<input type="button" param="${suppliers.totalPages}" flag="lastPage" value="尾页"/>&nbsp;<span>转到第<input name="basedataQuery.currentPage" id="pageNo" type="text" value="${basedataQuery.currentPage}" size="4" style="height:16px; width:28px; border:1px solid #999999; background:#FFF; border-radius:0;" />页 </span>&nbsp;&nbsp;<input type="button" value="转" style=" padding:0 10px;"/>
	</span>&nbsp;&nbsp;共有${suppliers.totalRows}条记录，当前第 ${suppliers.currentPage}/${suppliers.totalPages}页</p>
        </td>
    </tr>
</table>
</s:form>
<div>
    <a href="supplierAction_addUI.action">
